...
def generate_random_small_query(bound_x: int, bound_y: int) -> Tuple[Point, Point]:
    ...
    p2 = Point(random.randint(p1.x, min(int(p1.x + bound_x * 0.3), bound_x - 1)),
               random.randint(p1.y, min(int(p1.y + bound_y * 0.3), bound_y - 1)))
    # print(p1,p2)
    # Guarantee that query satisfies "dominates" assumption:
    if p1.x > p2.x:
        p1.x, p2.x = p2.x, p1.x
...
def generate_random_target_query(bound_x: int, bound_y: int, target: int) -> Tuple[Point, Point]:
    ...
    p2 = Point(min(random.randint(p1.x, p1.x + int(bound_x * target * 0.03)), bound_x - 1),
               min(random.randint(p1.y, p1.y + int(bound_y * target * 0.03)), bound_y - 1))
