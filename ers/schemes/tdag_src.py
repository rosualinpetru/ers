...
class TdagSRC(EMM):
    ...
    def build_index(self, key: bytes, plaintext_mm: Dict[Point, List[bytes]]):
        self.tree_product = HyperTreeProduct(self.emm_engine.DIMENSIONS_BITS, UniformSplitMidOverlapDivider(2))
    ...
    def trapdoor(self, key: bytes, query: HyperRange) -> Set[bytes]:
        ...
        hyperrange = self.tree_product.generate_src_cover(query)
        label = hyperrange.to_bytes()
